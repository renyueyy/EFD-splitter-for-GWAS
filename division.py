...
def black_division(input_path, output_path):
    ...
    for m in name_list:
        ...
        for i, contour in enumerate(cnts):
            ...
            cv.imwrite(os.path.join(pic_out_path, "{}.jpg".format(count)), rotated_canvas)  # 保存切割图片
...
def blue_division(input_path, output_path):
    ...
    for m in name_list:
        ...
        pic_out_path = output_path+'/'+m+'/'#输出文件路径
        pic_out_path = os.path.join(output_path,m)
        if not os.path.exists(pic_out_path):
            os.makedirs(pic_out_path)
...
def white_division(input,output):
    ...
    :return:
    """
    name_list = os.listdir(input)  # 获得图片名称列表，作为循环列表
    # print(name_list)

    for m in name_list:
        pic_path = os.path.join(input, m)
        image = cv.imread(pic_path)  # 读入图片
        img = distortion(image)
        print(m)

        pic_out_path = os.path.join(output,m)#输出文件路径
        if not os.path.exists(pic_out_path):
            os.makedirs(pic_out_path)
        ...
        for i, contour in enumerate(cnts):
            area = cv.contourArea(contour)  # 计算包围形状的面积
            if area < 30000 or area > 180000:  # 过滤面积，选择>30000,<150000的对象
                continue
            count += 1
            x, y, w, h = cv.boundingRect(contour)  # 确定正外接矩形左下角坐标（x,y）、宽w和高h
            box = np.array([[x, y], [x + w, y],
